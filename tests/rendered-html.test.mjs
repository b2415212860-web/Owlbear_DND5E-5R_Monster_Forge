...
test("implements device-local favorites and contains no token workflow", async () => {
  const [appSource, styles, packageJson] = await Promise.all([
  ...
  assert.match(appSource, /magic-term/);
  assert.match(appSource, /titleEn="Traits"/);
  assert.match(styles, /\.ability-table/);
  assert.match(styles, /--stat-red:/);
  assert.match(styles, /--magic-blue:/);
  assert.match(styles, /\.magic-term/);
  assert.doesNotMatch(appSource, /vital-grid|cr-seal|ability-grid/);
