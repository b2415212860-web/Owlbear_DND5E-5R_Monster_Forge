...
- `catalog_category`：原怪物图鉴的一级目录分类；
- `catalog_path`：从原图鉴根目录到怪物 HTML 所在文件夹的完整路径；
...
界面目录树直接使用 `catalog_path`，不会把原文件夹结构压平：例如 5R 的成年白龙位于 `龙类 / 白龙`，霸王龙位于 `附录A / 恐龙`。`catalog_category` 仍用于资料卡标签和跨版本统计，但不再代替目录结构。
